...
public class FlatlandView extends View {
	...
	private static long DELAY_FOR_CLICK_FADE = 0;
		...
		try {
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				...
				if (clickedTrackable == null) {
					...
					postDelayed(action, DELAY_FOR_CLICK_FADE + 1);
					getContext().startActivity(new Intent(getContext().getApplicationContext(), WearMapMarks.class));
				} else {
